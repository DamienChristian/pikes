...
import { UserCalendar, CalendarMember } from "@/app/types";
import { UserPlus, Trash2, Shield, Edit3, Eye } from "lucide-react";
...
export function ShareCalendarDialog({
  ...
  }, [open, calendar, fetchMembers]);

  const handleAddMember = async () => {
    if (!calendar || !addInput.trim()) return;
    setIsAdding(true);
    try {
      const res = await fetch(`/api/calendars/${calendar.id}/share`, {
        method: "POST",
  ...
  const memberDisplayName = (m: CalendarMember) => {
